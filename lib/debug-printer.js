...
lines.push('    function locate(string,uint32) constant returns (bool);');

var sigLocate = signatureHash('locate(string,uint32)');

printers.forEach(function(kind) {
    var signature = 'print(uint32,' + kind + ')';
    lines.push('    function ' + signature + ' constant returns (bool success) { return true; }');
...
DebugPrinter.prototype.process = function(data) {
    ...
    if (sig === sigLocate) {
        var kind = null;
        var value = ethers.Interface.decodeParams([ 'where', 'line' ], [ 'string', 'uint32' ], '0x' + data.substring(10));
        ...
        var kind = lookup[sig];
        var value = ethers.Interface.decodeParams([ 'line', 'data' ], [ 'uint32', kind ], '0x' + data.substring(10));
