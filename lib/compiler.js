...
Code.prototype.connect = function(address, providerOrSigner) {
    var contract = new ethers.Contract(address, this.interface, providerOrSigner)
    contract.connect = function(providerOrSigner) {
        return new ethers.Contract(contract.address, contract.interface, providerOrSigner);
    }
    return contract;
}
